...
import { IExamResult, IExamSubmission, IQuestion, IQuestionResponse, ISubject } from '../../interfaces_integration/exam';
...
export class ExamComponent {
  ...
  constructor(private questionService: QuestionService, private router: Router,private toastr : ToastrService) {this.preventBackButton(); }
 ...
 */
  ...
  preventBackButton() {
    ...
    window.addEventListener('popstate', () => {
      ...
      Swal.fire({
        title: 'Warning',
        text: 'Navigating away will end your exam. Please submit the test first.',
        icon: 'warning',
 ...
 */
 ...
 this.markedForReviewCount = this.reviewedQuestions.filter(review => review).length;
  }
 ...
 */
   ...
   */
    ...
    // sendExamSubmission(): void {
    //   const responses: IQuestionResponse[] = this.questions.map((question, index) => {
    //     // Ensure selectedOptions are integers
    //     const selectedOptions = Array.isArray(this.userResponses[index])
    //       ? this.userResponses[index].map(option => Number(option)) // Convert each option to a number (integer)
    //       : [Number(this.userResponses[index])]; // Single choice also converted to an integer
    
    //     return {
    //       questionId: question.id, // Assuming each question has a unique ID
    //       selectedOptions: selectedOptions // Ensure this contains an array of integers
    //     };
    //   });
